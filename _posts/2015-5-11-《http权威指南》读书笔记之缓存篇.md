...
另外，可以通过html中的<meta http-equiv="cache-control" content="max-age: 4000">来指定该html文档的过期时间，但是不会影响除该html文档以外的文件（如JS,CSS,图片等）。
...
如果文件不在Cache-control/Expires的有效范围内，则该文档是需要再验证的（revalidation）。
